...
public class RPGSprite extends Sprite {
	...
	public static Animation createAnimations(int animationDuration, Sprite[] sprites, boolean repeat)
	{
		return new Animation(animationDuration, sprites, repeat);
	}
	...
	//TODO comment in readme (it is a extract sprite when no orientation is needed
	...
	//TODO ALSO COMMENT
	public static Sprite[] extractSpritesVert(String name, int nbFrames, float width, float height, Positionable parent, int regionWidth, int regionHeight){  	

		Sprite[] sprites = new Sprite[nbFrames];

		for(int i = 0; i < nbFrames; i++){
			sprites[i]  = new RPGSprite(name, width, height, parent, new RegionOfInterest(0, regionHeight*i, regionWidth, regionHeight));
		}
		return sprites;
	}
	
	
	public static Sprite[] extractSpritesVert(String name, int nbFrames, float width, float height, Positionable parent, int regionWidth, int regionHeight, Vector anchor){  	

		Sprite[] sprites = new Sprite[nbFrames];

		for(int i = 0; i < nbFrames; i++){
			sprites[i]  = new RPGSprite(name, width, height, parent, new RegionOfInterest(0, regionHeight*i, regionWidth, regionHeight), anchor);
		}
		return sprites;
	}
